...
public class ZooGame extends Game {
	...
	public void restart() { // 재시작
		...
		while(itrRect.hasNext()) {
			...
			frame.remove(removingRect.getZooLabel()); // 푯말들 다 제외(어쩌면 따로 분리해서 텍스트만 초기화할수도)
			for(AnimalLabel animal : removingRect.getAnimals()) {
				if(animal != null)
					frame.remove(animal); // 동물 우리에 있는 동물들 화면에서 제외
			}		
		}	
		
		initializeFields(); // 최중요 필드값 초기화(레이블 옮기는 데 큰 영향)
		zooRectangle.clear(); // 동물원 사각형 다 비워버림(새로 채우기 위해)
		animalLabels.clear(); // 동물 리스트 다 비워버림(새로 선정하기 위해)
		startingRectangle.removeAll(); // 시작할 때 배치하는 곳에 동물 리스트가 남아있을 때를 대비하여 전부 비움
		frame.getContentPane().repaint(); // 화면 반영
		start(); // 다시 시작. 다 비워버렸으므로 처음 시작이나 다름없다.
	}
...
//					targetRectangle.addAnimal(movingLabel);
	...
	public void swap(AnimalRectangle targetAnimalParent, AnimalLabel targetAnimal, int targetIndex) {
		...
		frame.getContentPane().repaint();
		initializeFields(); // swap 도중 오류가 나는 원인은 끝나고 초기화를 안 해줬기 때문
		// 끝나고 변수들 초기화 안 할 경우, 
	}
